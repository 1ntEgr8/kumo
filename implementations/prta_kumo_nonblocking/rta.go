...
type rta struct {
	...
	// Key: string (method name), Value: *sync.Map (map[*concreteTypeInfo]bool)
	MC sync.Map

	// MI maps method names to sets of interface types that require that method
	// Key: string (method name), Value: *sync.Map (map[*interfaceTypeInfo]bool)
	MI sync.Map

	// methodCounts tracks how many times each method appears in interfaces
	// Key: string (method name), Value: *int32 (count, use atomic operations)
	methodCounts sync.Map

	// Method value cache, to avoid lock contention
	methodValueCache sync.Map
}
...
func (r *rta) addInvokeEdge(site ssa.CallInstruction, C types.Type) {
	...
	cmethod := r.lookupMethod(C, imethod.Pkg(), imethod.Name())
...
func (a *RTA) Analyze(roots []*ssa.Function, buildCallGraph bool, numWorkers int) kumo.Result {
	...
	if reflectPkg := r.prog.ImportedPackage("reflect"); reflectPkg != nil {
		...
		r.reflectValueCall = r.lookupMethod(reflectValue.Object().Type(), reflectPkg.Pkg, "Call")
	}
	...
	if v := r.concreteTypes.At(C); v != nil {
		...
		if oldcinfo, loaded := r.concreteTypes.LoadOrStore(C, cinfo); !loaded {
			...
			for i := 0; i < mset.Len(); i++ {
				...
				val, _ := r.MC.LoadOrStore(methodName, &sync.Map{})
				ctypeSet := val.(*sync.Map)
			...
			for i := 0; i < mset.Len(); i++ {
				methodName := mset.At(i).Obj().Name()
				if val, ok := r.MI.Load(methodName); ok {
					itypeSet := val.(*sync.Map)
					itypeSet.Range(func(key, value any) bool {
						candidates[key.(*interfaceTypeInfo)] = true
						return true
					})
				}
			}
			...
			for iinfo := range candidates {
				if I := types.Unalias(iinfo.I).(*types.Interface); implements(cinfo, iinfo) {
...
func (r *rta) implementations(I *types.Interface) *interfaceTypeInfo {
	...
	if v := r.interfaceTypes.At(I); v != nil {
		...
		if oldiinfo, loaded := r.interfaceTypes.LoadOrStore(I, iinfo); !loaded {
			// Kumo optimization: Update method counts and find rarest method
			var rarestMethod string
			var rarestCount int32 = 1<<31 - 1 // max int32

			for i := 0; i < mset.Len(); i++ {
				methodName := mset.At(i).Obj().Name()

				// Increment method count atomically
				val, _ := r.methodCounts.LoadOrStore(methodName, new(int32))
				countPtr := val.(*int32)
				newCount := atomic.AddInt32(countPtr, 1)

				// Track rarest method
				if newCount < rarestCount {
					rarestCount = newCount
					rarestMethod = methodName
				}
			}

			// Kumo optimization: Index interface under rarest method in MI
			if rarestMethod != "" {
				val, _ := r.MI.LoadOrStore(rarestMethod, &sync.Map{})
				itypeSet := val.(*sync.Map)
			...
			// for i := 0; i < mset.Len(); i++ {
			// 	methodName := mset.At(i).Obj().Name()
			// 	if val, ok := r.MC.Load(methodName); ok {
			// 		ctypeSet := val.(*sync.Map)
			// 		// Count the size
			// 		size := 0
			// 		ctypeSet.Range(func(key, value any) bool {
			// 			size++
			// 			return true
			// 		})
			// 		if size < bestSize {
			...
			// 		}
			// 	} else {
			// 		// Method not found in any concrete type yet
			...
			bestMethod = mset.At(0).Obj().Name()

			// Ascertain set of concrete types that implement I
			// and update the 'implements' relation.
			if bestMethod != "" {
				if val, ok := r.MC.Load(bestMethod); ok {
					ctypeSet := val.(*sync.Map)
					ctypeSet.Range(func(key, value any) bool {
						cinfo := key.(*concreteTypeInfo)
						if implements(cinfo, iinfo) {
							...
							iinfo.implementations.SetNoLock(cinfo.C, struct{}{})
						}
						return true
					})
				}
...
func (r *rta) lookupMethod(T types.Type, pkg *types.Package, name string) *ssa.Function {
	...
	if cached, ok := r.methodValueCache.Load(key); ok {
		return cached.(*ssa.Function)
	}
	mset := types.NewMethodSet(T)
	sel := mset.Lookup(pkg, name)
	result := r.prog.MethodValue(sel)
	r.methodValueCache.Store(key, result)
	return result
}
